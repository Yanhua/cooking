...
const nav=()=>{
  const active=navPage();
  return `<nav class="bottom-nav" aria-label="Main navigation"><button type="button" data-nav="weeks" class="nav-button ${active==='weeks'?'active':''}" ${active==='weeks'?'aria-current="page"':''}><span class="nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><rect x="3" y="4.5" width="18" height="16" rx="3"></rect><path d="M8 3v4M16 3v4M3 9h18M8 13h.01M12 13h.01M16 13h.01M8 17h.01M12 17h.01M16 17h.01"></path></svg></span><span>Weeks</span></button><button type="button" data-nav="recipes" class="nav-button ${active==='recipes'?'active':''}" ${active==='recipes'?'aria-current="page"':''}><span class="nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M4 5.5A2.5 2.5 0 0 1 6.5 3H12v17H6.5A2.5 2.5 0 0 1 4 17.5zM20 5.5A2.5 2.5 0 0 0 17.5 3H12v17h5.5a2.5 2.5 0 0 0 2.5-2.5z"></path><path d="M7.5 7.5h2.5M7.5 11h2.5M14 7.5h2.5M14 11h2.5M4 17.5h16"></path></svg></span><span>Recipes</span></button></nav>`;
};
...
function render(keepScroll=false){
  ...
  const previousScroll=keepScroll?app.querySelector('.app-scroll')?.scrollTop||0:0;
  app.innerHTML=`<fieldset class="app-controls" ${busy?'disabled':''}><div class="app-scroll">`+html+ (notice?`<p class="notice" role="status">${esc(notice)}</p>`:'') +(removedPlan?'<div class="content"><button class="secondary" data-action="undo-remove">Undo removal</button></div>':'')+`<section class="content cloud-controls"><p id="sync-status" class="sub" role="status">${busy?'Saving…':'Connected to your household'}</p><button class="secondary" data-action="lock">Lock planner</button>${incoming?'<button class="secondary" data-action="refresh-cloud">Load latest saves</button>':''}${legacy && (legacy.saved.length || legacy.draft?.ids.length || Object.keys(legacy.tried).length)?'<button class="secondary" data-action="import-local">Import this browser’s old saves</button>':''}</section></div>`+nav()+`</fieldset>`;
